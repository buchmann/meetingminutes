...
_tracer = trace.get_tracer("local_ai.text_improver")
...
async def _call_openai(
    ...
    ``temperature`` is the "hallucination" dial; None uses the default 0.3.
    """
...
async def _call_ollama(
    ...
    """Call Ollama for text improvement. None temperature uses default 0.3."""
    temp = 0.3 if temperature is None else temperature
...
async def improve_text(
    ...
    *,
    ...
    ``temperature`` is the "hallucination" dial (None → default 0.3).
    ...
    try:
        if backend == "openai":
            raw = await _call_openai(
                prompt, openai_base_url, openai_api_key, openai_model, temperature,
            )
        else:
