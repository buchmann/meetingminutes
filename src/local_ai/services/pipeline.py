...
from local_ai.models import TranscriptResult

logger = logging.getLogger(__name__)
tracer = trace.get_tracer("local_ai.pipeline")
            ...
            try:
                ...
                if cached_transcript:
                    ...
                    with tracer.start_as_current_span("pipeline.preprocess") as span:
                        ...
                        from local_ai.services.audio import preprocess_audio
                    ...
                    if self.settings.transcription_backend == "remote":
                        ...
                        with tracer.start_as_current_span(
                            ...
                            from local_ai.services.remote_transcriber import remote_transcribe
                        ...
                        with tracer.start_as_current_span(
                            ...
                            from local_ai.services.transcriber import transcribe
                        ...
                        from local_ai.services.transcriber import clear_model_cache
                        ...
                        if job["diarization_on"] and self.settings.hf_token:
                            with tracer.start_as_current_span(
                                ...
                                from local_ai.services.diarizer import diarize
                        ...
                        with tracer.start_as_current_span("pipeline.merge"):
                            ...
                            from local_ai.services.merger import merge_transcript_diarization, segments_without_diarization
                ...
                if job["summarization_on"]:
                    try:
                        ...
                        with tracer.start_as_current_span(
                            ...
                            from local_ai.services.summarizer import summarize
                            ...
                            summary_result = await summarize(
                                ...
                                style_profile=style,
                                detail_level=job.get("summary_detail") or "standard",
                            )
                            summary_json = summary_result.model_dump_json()
                            span.set_attribute("summary.language", summary_result.language)
                            span.set_attribute("summary.detail", job.get("summary_detail") or "standard")
