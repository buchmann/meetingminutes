...
from local_ai.models import JobResponse
...
async def create_job(
    ...
    summary_detail: str = Form(default="standard"),
...
async def resummarize_job(
    request: Request,
    job_id: str,
    summary_detail: str = Form(default=""),
    user: dict = Depends(require_user),
):
    """Re-run summarization on a completed job (owner only).

    Optional form field ``summary_detail`` (``standard`` | ``detailed``) overrides
    the job's stored detail level — useful for re-running the same transcript
    with the other depth without uploading again.
    """
    db = request.app.state.db
...
async def recording_devices(user: dict = Depends(require_user)):
    ...
    from local_ai.services.recorder import list_audio_devices
...
async def analyze_style_from_text(request: Request, user: dict = Depends(require_user)):
    ...
    from local_ai.services.style_analyzer import analyze_style
